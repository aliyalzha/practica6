...
filterLabel.addEventListener('click', function() {
	...
	if (filterIcon.getAttribute('src') === filter.png) {
		filterIcon.setAttribute('src', 'filterhover.png')
	} else {
		filterIcon.setAttribute('src', 'filter.png')
	}
});
...
let filterSizesWrap = document.querySelector('.filterSizesWrap');
filterSizesWrap.addEventListener('click', function(){
...
class cart {
	...
	addToCart(product) {
		var existInCart = this.products.map(function(productSearch) {
			return productSearch.name; }).index0f(product.name);
		if (existInCart >= 0) {
...
let itemsCart = document.querySelector(".itemCart");
itemsCart.textContent = cart.countProducts.toString();

let addBtns = document.querySelectorAll(".filterAddToCart");
...
function generateCartView(itemsCart) {
	...
	if (popupExist !=undefined) {
		...
		generateCartView(cart, cartTable);
	...
	function generateCartTable(cart, cartTable){
		...
		tableHTML += `<tr> <th>Итог:</th> <th>${cart.countProducts}</th> <th>$${cart.fullPrice}</th> </tr>`;
